...
describe('WordEditDialogComponent', () => {
  ...
  it('should require only the word field', () => {
    ...
    expect(component.wordForm[WordParameterEnum.WORD]().getError('required')?.message).toBe(
      'Word is required',
    );
    expect(
      component.wordForm[WordParameterEnum.TRANSLATION]().getError('required'),
    ).toBeUndefined();
  ...
  it('should create a custom group before adding a word', () => {
    mockWordGroupService.addGroup.mockReturnValue(
      of({
  ...
  it('should convert empty optional fields to undefined before submitting', () => {
    ...
    expect(mockWordsService.addWord).toHaveBeenCalledWith({
      ...
      [WordParameterEnum.GROUP_ID]: undefined,
    });
  });
  ...
  function setFormValue(value: {
    ...
    component.wordForm[WordParameterEnum.TRANSLATION]().value.set(
      value[WordParameterEnum.TRANSLATION],
    );
    component.wordForm[WordParameterEnum.DESCRIPTION]().value.set(
      value[WordParameterEnum.DESCRIPTION],
    );
