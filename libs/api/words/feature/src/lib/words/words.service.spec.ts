...
describe('WordsService', () => {
  ...
  const service = new WordsService(wordModel as never, collectionModel as never);
  ...
  it('should hide missing or foreign words behind a not-found response', async () => {
    ...
    await expect(
      service.update(new Types.ObjectId(), { translation: 'кіт' }, new Types.ObjectId().toString()),
  ...
  it('should delete only words owned by the authenticated user', async () => {
    ...
    await expect(service.remove([wordId.toString()], userId.toString())).resolves.toEqual({
  ...
  it('should copy words from shared collections without assigning a collection', async () => {
    ...
    await expect(service.copy([wordId.toString()], userId.toString())).resolves.toEqual([
      copiedWord,
    ]);
